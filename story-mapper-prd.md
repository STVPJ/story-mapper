...
- **Border:** 1px solid light grey (`#E5E7EB`), with subtle left-side colour accent matching the parent Release colour
...
- Changes auto-save on each field blur (no explicit save button needed)
...
- A drop indicator (highlighted line or gap) shows where the item will land.
- Invalid drop zones are visually distinct (e.g. greyed out or no indicator shown).
- The dragged card follows the cursor with a slight offset and rotation.
...
- Each map has options to rename, duplicate, or delete
...
The app requires two environment variables (set in Vercel's project settings):
...
- Bulk reorder operations (e.g. after drag-and-drop) should batch-update the `order` column for all affected rows in a single call where possible.
...
- **Import** uploads a JSON file, validates the structure, and inserts the data into Supabase as a new story map with new UUIDs (to avoid ID conflicts). The `user_id` on all imported records is set to the currently authenticated user.
...
- Debounce bulk reorder writes (e.g. after drag-and-drop changes multiple `order` values) using a 300ms timeout before sending to Supabase.
- All Supabase operations should set `user_id` to the currently authenticated user's ID from `supabase.auth.getUser()`.
...
- If an imported JSON file is malformed, show a clear error message and do not insert any data
